...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = {
      event_id: safeText(d.event_id, 120), join_id: joinId, session_id: safeText(d.session_id, 200),
    ...
    const upstream = await collectorPost(payload);
    return json(res, 200, { ok: true, event_id: payload.event_id, upstream: upstream.ok === true });
  } catch (error) {
    console.error('event proxy failed', error);
    return json(res, 502, { ok: false, error: 'Event could not be recorded' });
  }
}
